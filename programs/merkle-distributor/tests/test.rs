...
fn test_pda_derivation() {
    ...
    let version = 0u64;
    let address_tree = Keypair::new().pubkey();
    ...
    let (claim_status_pda, _bump) =
        get_claim_status_pda(&PROGRAM_ID, &claimant, &distributor_pda, &address_tree);
...
async fn test_distributor_integration_with_light_program_test() {
    ...
    // Initialize LightProgramTest
    let config = ProgramTestConfig::new(true, Some(vec![("merkle_distributor", PROGRAM_ID)]));
    ...
    // Setup address merkle context for compressed accounts
    ...
    // Setup packed accounts for new claim
    let mut packed_accounts =
        PackedAccounts::new_with_system_accounts(SystemAccountMetaConfig::new(PROGRAM_ID));
    pack_address_merkle_context(
        &address_merkle_context,
        &mut packed_accounts,
        proof.addresses[0].root_index,
    );
    packed_accounts.insert_or_get(rpc.test_accounts.v1_state_trees[0].merkle_tree);
